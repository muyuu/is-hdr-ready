...
import { selector, onlySupported, onlySupportedKeySystem, lessThanLevel } from './selector.js';
...
function main() {
    const pairs = [
        { key: 'container', id: 'containerFilterSelect'},
        { key: 'codec', id: 'codecFilterSelect'},
        { key: 'type', id: 'supportedFilterSelect'},
        { key: 'level', id: 'levelFilterSelect'},
    ];
    selector(pairs);
    put();
}
...
async function put() {
    const containerFilter = document.getElementById('containerFilterSelect');
    const codecFilter = document.getElementById('codecFilterSelect');
    ...
    for (let codec of codecs) {
        ...
        const r = await check({ codecStr, container });

        const { searchParams } = new URL(location.href);
        const pageType = searchParams.get('type');
        const levelParam = parseFloat(searchParams.get('level'), 10);
        ...
        if ( lessThanLevel(levelParam, level) ) {
            continue;
        }
